...
{
	//alert( "submitting" );
	document.userinput.theChar.value = escape(theChar);
	return (false);
}
...
function mygetKey( event )
{
   var key;
	...
	//alert( event.value );
	if( event.value != "" && event.value != theChar ) {
		theChar = event.value;
		theCharList = theChar;
		theCharIndex = 1;
			...
			for( i = 0 ;meat && i < meat.length ; ++i ) {
				...
				if( meat[i].childNodes[0] ) {
					...
					if(  meat[i].childNodes[0].nodeName == "gb" && meat[i].childNodes[0].childNodes[0] ) {
						...
						//alert( "0.5" );
						catstr += "<a href=\"http://www.learn-chinese-words.com/cgi-bin/external.cgi?extsrc=dictcn&charstring=";
						//alert( "1" );
...
function initpage() 
{
	...
	if( tmpInt != -1 ) {
		...
		theChar = unescape( unescape( tmpStr ));
		theCharList = theChar;
		theCharIndex = 1;
		theCommonElement =  theChar ;
		theCharCode = escape( theChar );
